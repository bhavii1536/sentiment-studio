...
def detect_emotions(texts):
    emotions = []
    for t in texts:
        res = emotion_model(t[:512])
        emotions.append(max(res, key=lambda x: x["score"])["label"].capitalize())
    return emotions
...
def show_sentiment_charts(sentiments):
    ...
    with col2:
        ...
        s.plot(kind="barh", color=PASTEL_COLORS, ax=ax)
...
if mode == "Product / Topic Analysis (YouTube)":
    ...
    if st.button("Analyze"):
        ...
        st.caption("✨ Almost there… gathering real voices")
        ...
        if comments:
            ...
            emotions = detect_emotions(comments)
...
# ===============================
    ...
    if st.button("Analyze Channel"):
        ...
        if res["items"]:
            ...
            c1, c2 = st.columns(2)
            c1.metric("Subscribers", stats["subscriberCount"])
            c2.metric("Total Views", stats["viewCount"])
